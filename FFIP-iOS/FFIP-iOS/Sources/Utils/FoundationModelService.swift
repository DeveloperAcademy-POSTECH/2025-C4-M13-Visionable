...
//    }
    ...
    func findRelatedKeywords(
        ...
        let response = try await session.respond(generating: RelatedKeywords.self) {
            ...
            recognitionKeywords 배열에 있는 키워드에서 "\(searchKeyword)"와 가장 유사한 단어를 찾아 "findKeyword" 필드에 담아주고, 
            유사도를 similarity 필드에 0~1 범위의 수치로 반환해줘.
            
            만약 "\(searchKeyword)"와 유사한 단어가 없다면 "findKeyword" 필드에 nil을 담고, similarity는 0을 반환해줘.
            """
        }
